...
class LSLClient(_BaseClient):
    ...
    def get_data_as_epoch(self, n_samples=1024, picks=None):
        ...
        wait_time = n_samples * 5. / 1000#self.info['sfreq']
    ...
    def _create_info(self):
        ...
        for _t in range(len(ch_types)):
            if ch_types[_t]=='impedance' and _t<3:
                ch_types[_t] = 'eeg'
            elif ch_types[_t]=='impedance':
                ch_types[_t] = 'grad'
            if ch_types[_t]=='acc':
                ch_types[_t] = 'grad'
            elif ch_types[_t]=='packet' or ch_types[_t]=='trig':
                ch_types[_t] = 'stim'
        print(ch_type, ch_names, ch_types, ch_info)
        if ch_type == "impeadance":
